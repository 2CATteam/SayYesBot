...
class Bot {
    ...
    static checkMessage(message) {
		...
		"It's a good thing you're learning C++ - it doesn't have automatic garbage collection, so maybe it'll stop deleting your code.",
		"I've found the only way to get your code to compile is to comment it all out... If only I could do the same to you.",
		"Scientists have found it too complex to create an AI with the intelligence of a six-year-old, so they've readjusted their goal. Now they just hope to make an AI with your intelligence."
		]
		...
		const noRegex = /^say no/;
		
		const bigRegex = /^big$/;
		
		const sayOofRegex = /^say\s*oof/
		...
		if (messageText && (message.sender_type == "user"))
		{
			...
			if (profRegex.test(messageText.toLowerCase()))
			{
				...
				toReturn += " ";
				toReturn += profArray[randomInt];
				return toReturn;
			}
			if (noRegex.test(messageText.toLowerCase()))
			{
				return "Wow, you're so clever...";
			}
			if (bigRegex.test(messageText.toLowerCase()))
			{
				return "OOF";
			}
			if (sayOofRegex.test(messageText.toLowerCase()))
			{
				return "OOF";
			}
		}
